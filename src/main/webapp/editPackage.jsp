...
<div class="edit-package-container">
    ...
    <div class="page-header">
        ...
        <a class="btn btn-secondary" href="/erpflow/packages.jsp">Back to Packages</a>
    </div>
    ...
    <div class="card">
        ...
        <div class="package-info">
            <p><strong>Sales Order:</strong> <span id="salesOrderNumber">—</span></p>
            <p><strong>Status:</strong> <span id="packageStatus">—</span></p>
        </div>
        ...
        <form id="editPackageForm" novalidate>
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="length" min="0.01" step="0.01" required>
                </div>

                <div class="form-group">
                    <label for="width">Width (cm)</label>
                    <input type="number" id="width" min="0.01" step="0.01" required>
                </div>

                <div class="form-group">
                    <label for="height">Height (cm)</label>
                    <input type="number" id="height" min="0.01" step="0.01" required>
                </div>
            </div>

            <h2>Package Items</h2>
            <p class="muted">
                Change quantities below. Each item must remain associated with its original Sales Order line.
            </p>
            ...
            <div id="editPackageError" class="error-message" role="alert" style="display:none;"></div>
            <div id="editPackageSuccess" class="success-message" role="status" style="display:none;"></div>

            <div class="form-actions">
                <button type="submit" class="btn btn-primary" id="savePackageButton">
                    Save Changes
                </button>
                <a class="btn btn-secondary" href="/erpflow/packages.jsp">Cancel</a>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    async function loadPackage() {
        ...
        if (!packageId || !/^[1-9]\d*$/.test(packageId)) {
            showError("A valid package ID is required in the URL. Example: editPackage.jsp?id=1");
            form.style.display = "none";
            return;
        }

        try {
            const response = await fetch(`/erpflow/api/packages/${encodeURIComponent(packageId)}`);
            ...
            if (!isEditable(loadedPackage.status)) {
                form.style.display = "none";
                showError("This package cannot be edited because its status is not PACKED.");
                return;
            }
    ...
    form.addEventListener("submit", async function (event) {
        ...
        if (Object.values(dimensions).some(value => value === null)) {
            showError("Weight and all dimensions must be valid numbers greater than zero.");
            return;
        }
        ...
        for (const input of itemsBody.querySelectorAll(".quantity-input")) {
            ...
            const itemId = original && original.item ? Number(original.item.id) : NaN;
            const salesOrderItemId = Number(original && original.salesOrderItemId);

            if (!Number.isSafeInteger(itemId) || itemId <= 0 ||
                !Number.isSafeInteger(salesOrderItemId) || salesOrderItemId <= 0) {
                showError("An item or Sales Order line has invalid data. Reload the package and try again.");
                return;
            }
